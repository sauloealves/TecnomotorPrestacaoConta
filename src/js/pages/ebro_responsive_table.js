	...
	ebro_responsive_table = {
		init: function() {
			if($('#tblLancamento').length) { 
				$('#tblLancamento').footable().on('footable_filtering', function (e) {
				...
				$('#clear-filter').click(function (e) {
					e.preventDefault();
					$('#un_member').val('');
					$('#tblLancamento').trigger('footable_clear_filter');
				});
				
				//* apply all filters on select change
				$('#un_member').change(function (e) {
					e.preventDefault();
					$('#tblLancamento').trigger('footable_filter', {filter: $('#table_search').val()});
				});	
			}
		}
	}