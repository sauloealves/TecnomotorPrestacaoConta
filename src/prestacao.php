	...
	{		
		...
		{		
			...
			{				
				...
				//Verifica se o usuário está habilitado para acessar os dados do cartão passado por parametro
			...
			<div id="main_content_outer" class="clearfix">
				<div id="main_content">
						...
						if ($QtdeCartoes > 1){
								$addlinha = $addlinha."<div class='col-sm-6'>";
									...
									$addlinha = $addlinha."<label for='reg_select'>Selecionar outro cartao</label>";
										...
										$addlinha = $addlinha."<option value='' disabled selected>Selecionar outro cartão</option>";
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="panel panel-default">
								<div class="panel-heading">
									<h4 class="panel-title"><label id="lblTituloCartao">Lançamentos do cartão: XXXX-XXXX-XXXX-<?php print_r($numeroCartao) ?></label></h4>	
								</div>
						...
						<div class="modal fade" id="modalBuscaContaContabil">
							<div class="modal-dialog" style="width: 70%">
								<div class="modal-content">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal">&times;</button>
										<h4 class="modal-title">Busca conta contábil</h4>
									</div>
									<div class="modal-body">
						...
						<div class="col-sm-12">
							<?php echo("<td colspan='4'><label id='lblImpressao'><a href='#' onclick='imprimir()'>Versão para impressão</a><label></td>") ?>
						</div>
	...
	<script type="text/javascript">
		...
		function mudarcontacontabil(empresa, id, pk_lancamento){
			$.ajax({
				...
				method: 'POST',
				success: function(data){
					$.stickyNote('Conta contábil salva com sucesso.', $.extend({}, defaults, {autoclose: 2000, classList: 'stickyNote-success',position: 'top-center'}), callback);
		...
		$(function(){
			...
			$.ajax({
				...
				error: function (result, b,c){
					$("#lblProprietarioCartao").text("Proprietário do cartão: " + result.responseText);
				}
			});
			...
			$.ajax({
				...
				success: function(data){
					...
					if (($("#ddlCartoes").val() == null) == true){ // caso seja usuario de um cartao só
